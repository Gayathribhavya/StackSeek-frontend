...
export default function Login() {
  ...
  const [loginSession, setLoginSession] = useState(null); // Store session info if needed

  // Access authentication helpers from context
  const { login: loginUser, loginWithGoogle, loginWithGitHub, loginWithMicrosoft } = useAuth();

  // Helper to check if 2FA is enabled for the user
  async function check2FAEnabled(email) {
    try {
      const res = await fetch("/api/repository/user-security", {
        method: "POST",
  ...
  // Helper to verify 2FA code
  async function verify2FACode(code) {
    try {
      const res = await fetch("/api/repository/2fa/login-verify", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const is2FAEnabled = await check2FAEnabled(email);
      if (is2FAEnabled) {
        setShow2FA(true);
      } else {
  ...
  const handle2FASubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const success = await verify2FACode(twoFACode);
      if (success) {
        window.location.href = "/connect-repository";
      } else {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // Google accounts are always verified. Navigate directly to connect
      // repository.
      window.location.href = "/connect-repository";
    } catch (error: any) {
      console.error(error);
  ...
  const handleMicrosoftLogin = async () => {
    ...
    try {
      ...
      // Microsoft accounts are verified through Firebase OAuth. Navigate directly to connect repository.
      window.location.href = "/connect-repository";
    } catch (error: any) {
      console.error(error);
      alert(error?.message || "Microsoft sign in failed");
    } finally {
  ...
  const handleGitHubLogin = async () => {
    ...
    try {
      ...
      // GitHub accounts redirect is handled in the auth context
      window.location.href = "/connect-repository";
    } catch (error: any) {
      console.error(error);
        ...
        <Card className="w-full max-w-md transition-all duration-300 hover:shadow-lg animate-in fade-in slide-in-from-bottom-4 duration-1000">
          ...
          <CardContent className="space-y-6">
            ...
            <div className="flex flex-col items-center justify-center text-center pt-0 pb-2 px-2">
              <CardTitle className="text-2xl font-bold mb-2 leading-tight">Welcome back</CardTitle>
              ...
              <form onSubmit={handle2FASubmit} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className="transition-all duration-200 border border-gray-300 hover:border-blue-400 focus:border-blue-500 focus:ring-0 focus:outline-0 focus:shadow-none rounded-md bg-transparent dark:bg-transparent dark:border-gray-600 dark:hover:border-blue-400"
                    required
                  />
                </div>
                <Button
